...
Session Start (track selected: technical | HR)
   │
   ├──► [if camera_enabled] Client-side Delivery Analytics begins
   │        (MediaPipe Face/Pose + Web Audio, in-browser, no upload)
   │
   ▼
...
Loop until N questions or time cap reached
   │
   ├──► Client finalizes delivery metrics locally ──► POST /interview/session/{id}/delivery-metrics
   │
   ▼
AI Gateway: interview.feedback (context now includes delivery_metrics summary alongside transcript + profile)
   │
   ▼
Feedback Report persisted (now includes delivery-informed communication_notes) ──► Event Log: interview.session_completed (payload includes camera_enabled flag)
   │
   ▼
Readiness sub-score updated (now a blend of transcript-based + delivery-based signal — see rules.md changes) ──► Dashboard
   │
   ▼
Suggested missions from feedback ──► queued into Mission Engine (next mission may reference gaps found, including delivery-specific remedial missions)
```
...
## 7. Psychometric Profiling Data Flow

```
Onboarding (or post-onboarding prompt)
   │
   ▼
Learner completes instrument answers (IPIP + RIASEC items)
   │
   ├── Consent check: dedicated psychometric consent record must exist (rules.md §9)
   │
   ▼
POST /profile/psychometrics
   │
   ├── Deterministic Scoring Service (psychometrics_service.py)
   │     ├── IPIP scoring: item-to-scale mapping + reverse coding → 5 trait scores (0-100)
   │     └── RIASEC scoring: item-to-scale mapping → 6 dimension scores (0-100)
   │
   ▼
AI Gateway: psychometrics.narrate (input = numeric scores, output = narrative + hints)
   │
   ▼
psychometric_profiles row persisted (scores, narrative, hints, consent_id, instrument_version)
   │
   ▼
Roadmap Generation (roadmap.generate) — receives optional psychometric context block
   │
   ├── If psychometric context present: pacing_hint + tone_hint influence mission pacing and copy tone
   │
   └── If psychometric context absent: roadmap generation works unchanged (graceful degradation)
```

**Key properties:**
- Scoring is deterministic — the LLM never computes scores, only narrates them.
- Psychometric data flows into roadmap generation as an **optional enrichment**, never as a blocking input.
- A learner who skips the assessment gets a normal, unaffected roadmap (graceful degradation, per `prd.md` non-goals).
