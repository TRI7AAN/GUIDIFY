...
| FR15 | Interview Delivery Analytics (client-side eye contact, posture, vocal pacing, filler-word detection, longitudinal trend tracking) | P1 (post initial Interview Bot ship) |
| FR16 | Psychometric Profiling (Big Five personality traits via IPIP short-form + RIASEC career interest codes, deterministic scoring, optional during onboarding) | P2 |
...
| Camera opt-in rate on interview sessions | ≥ 50% |
| Delivery-metric-driven mission completion rate | ≥ 40% of learners who complete a delivery-flagged remedial mission |
| Measured eye-contact/pacing improvement over 3+ sessions | Positive trend for ≥ 60% of repeat users |
| Psychometric assessment completion rate (of onboarding starters) | ≥ 40% |
| Roadmaps generated with psychometric context present | ≥ 30% of active learners |
...
- **Camera-based analytics:** may see lower opt-in from privacy-conscious users or low-bandwidth/low-end-device users — client-side-only processing mitigates but does not eliminate this; track opt-in rate closely before expanding scope (e.g., video storage, coaching replays).
- **Psychometric instrument validity:** Big Five (IPIP) and RIASEC are well-validated instruments, but interpretation quality depends on prompt design (`psychometrics.narrate`). Instrument validity and narrative output quality should be reviewed periodically — flag for expert review if trait-profile-to-roadmap-pacing correlation data suggests the narration is not producing actionable differentiation.
- **Non-goal — opportunity gating:** Psychometric data must never be used to tell a learner a career path "isn't for them," restrict which roles/roadmaps are available, or appear anywhere near rejection/discouragement messaging. This is pacing/tone context only — a hard rule, not a style preference (cross-reference `rules.md`).
